...
## 发展
![](../../assets/images/UESTC_OS_type_time_line.png)
...
### 实时系统
对计算机响应时间有要求的(可预测)系统就是实时系统, 实时系统并不是反应很快的系统, 而是反应具有时序可预测性的系统

### 现代操作系统
- 个人操作系统中每个用户可能会运行多个程序, 所以加入了 **分时** 机制对程序运行进行改善
- 网络操作系统: 将网络中的多个计算机虚拟为一个计算机
- 分布式操作系统: 将计算分配到多个计算机中, 整合计算机资源
...
#### 多模式结构风格
包含多个 ***模式模块***, 不同的模块 **在不同的 `CPU` 特权模式** 下运行, 有利于系统的稳定性

#### 双模式基础平台子系统
**双模式** 是指基础平台子系统中包含两个模式模块, 分别运行在不同的 `CPU` 特权模式下;

![](../../assets/images/UESTC_OS_struct_double_mode.png)

- 核外子系统使用的 `CPU` 特权称为 `User Mode`
- 核心子系统使用的 `CPU` 特权称为 `Kernel Mode`

##### 微核结构
`Micro Kernel` 风格尽量将核心子系统中的多余部分放到核外子系统中实现, `Kernel` 只实现简单和必要的功能, 保持简洁高效
